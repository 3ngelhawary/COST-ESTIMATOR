...
(function () {
  ...
  function bind() {
    ...
    $("projectName").oninput = (e)=>{
      st.projectName = e.target.value;
      window.UIRender.preview();
    };

    // ✅ FIX: no re-render while typing
    $("projectAreaSqm").oninput = (e)=>{
      st.projectAreaSqm = e.target.value;
      window.UIRender.preview();
    };

    $("durationMonths").oninput = (e)=>{
      st.durationMonths = e.target.value;
      window.UIRender.preview();
    };
    ...
    if (pl) {
      pl.oninput = (e)=>{
        st.projectLengthManual = e.target.value;
        window.UIRender.preview();
      };
    }

    $("drawingScale").onchange = (e)=>{
      st.drawingScale = e.target.value;
      window.UIRender.preview();
    };
    ...
    $("facilityInfoBtn").onclick = ()=>{
      window.UIFacilityInfo.openFacilityTable();
    };
