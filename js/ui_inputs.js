...
(function () {
  ...
  function renderInputs() {
    ...
    if (!st.requiredDetails) {
      st.requiredDetails = { concept:false, schematic:false, detail:false, shop:false, asbuilt:false };
    }
    ...
    $("inputs").innerHTML = `
      ...
      <div class="row">
        ...
        <div class="field">
          ...
          <select id="drawingScale">
            ${window.APP_CONFIG.drawingScales.map(s=>`<option ${st.drawingScale===s?"selected":""}>${s}</option>`).join("")}
          </select>
        </div>
      </div>
      ...
      <div class="toggleRow">
        <div class="left">
          ...
          <div class="s">${st.bimRequired ? "BIM" : "CAD"}</div>
        </div>
        <label class="switch">
