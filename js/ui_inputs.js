...
(function () {
  ...
  function renderInputs() {
    ...
    const lenVal  = st.lengthOverride
      ...
      : (autoLen ? String(Math.round(autoLen * 100) / 100) : "");
    ...
    $("inputs").innerHTML = `
      ...
      <div class="field">
        <label>Project Length (m) <span class="smallNote" style="display:inline;margin:0;">(auto = 20% of area)</span></label>
        <input id="projectLength" type="number" min="0" step="0.01"
               value="${lenVal}" ${st.lengthOverride ? "" : "disabled"}>
        <label style="display:flex;gap:8px;font-size:12px;margin-top:6px;color:var(--muted);cursor:pointer;align-items:center;">
      ...
      <div class="row">
        ...
        <div class="toggleRow">
          <div class="left">
            <div class="t">Workflow</div>
            <div class="s">${st.bimRequired ? "BIM (LOD)" : "CAD"}</div>
          </div>
          <label class="switch">
      ...
      <div class="row">
        <div>
          ...
          <div class="checkGrid">${grid(window.APP_CONFIG.disciplines.wet, "wet", st.disciplines.wet)}</div>
        </div>
        <div>
          <div class="groupTitle"><h3>Dry Utilities</h3></div>
          <div class="checkGrid">${grid(window.APP_CONFIG.disciplines.dry, "dry", st.disciplines.dry)}</div>
        </div>
      </div>
      ...
      <button type="button" class="fullWidth" id="facilityInfoBtn">
        ⊞ Open Facility Table
        ${st.facilities.length ? `(${st.facilities.length} row${st.facilities.length>1?"s":""})` : ""}
      </button>
    `;
  }
